...
def test_validate_repo_accepts_well_formed_owner_repo():
...
def test_validate_repo_rejects_malformed_input(bad_repo):
    with pytest.raises(InvalidRepoError):
        validate_repo(bad_repo)


def test_list_commits_validates_eagerly_not_on_iteration():
...
def test_pagination_follows_link_header_across_pages():
...
def test_404_raises_repo_not_found_error():
...
def test_rate_limited_403_raises_rate_limit_error_with_reset_time():
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            403,
...
def test_non_rate_limit_403_raises_generic_http_error():
